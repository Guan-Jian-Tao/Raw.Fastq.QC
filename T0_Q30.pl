...
foreach my $f (@files) {
	...
	my $out1 = $opt{outdir}.$p."_1.Q20.Q30.txt";
	my $out2 = $opt{outdir}.$p."_2.Q20.Q30.txt";
	my $sh = "Q30.Stat.$p.sh";
	open (OUT,">$sh");
	print OUT "\#\$ \-S \/bin\/bash\n\#\$ \-j y \-V\n\#\$ \-cwd\n\#\$ \-l h\_vmem\=20G\n\#\$ -q all.q\n";
	sleep 0.1;
	print OUT "time1=`date \"+%Y-%m-%d %H:%M:%S\"` \necho T0_Q30.pl for $p starts\ at \$time1  >> $opt{logfile}\n";
	print OUT "#Main Command Line\n########\n";
	print OUT "python q30-master/q30.mod.py $fq1 > $out1 \n";
	print OUT "python q30-master/q30.mod.py $fq2 > $out2 \n";
	print OUT "########\n";
	print OUT "time2=`date \"+%Y-%m-%d %H:%M:%S\"`\n";
	print OUT "end_dat=`date -d \"\$time2\" +%s` \nstart_dat=`date -d \"\$time1\" +%s` \ninter_s=`expr \$end_dat - \$start_dat` \ninter_m=`expr \$inter_s / 60` \necho T0_Q30.pl for $p completes at \$time2 and consumed \$inter_m minutes >> $opt{logfile} \n";
	close OUT;
	system "chmod 700 $sh";
	system "qsub $sh";
	sleep 0.1;
	print "python q30-master/q30.mod.py $fq1 > $out1 \npython q30.mod.py $fq2 > $out2 \n";
	sleep 0.2;
}
